...
//!
//! **在做什么**：菜谱（字节码）已经写好，这里一步一步执行。
//!
//! **栈帧（Frame）白话**  
//! 每次调用函数就压入一个 Frame，记录：
//! - `func`：正在执行哪个函数
//! - `ip`：下一条指令在哪（指令指针）
//! - `base`：这个函数的局部变量从栈的哪一格开始
//!
//! 例如调用 `f(10)`：栈上先有参数 `10`，`base` 指向它；  
//! 函数里的第一个局部槽就是 `stack[base + 0]`。
//!
//! **Return 约定**  
//! - `void` 函数：把栈截断到 `base`（参数和局部都清掉）
//! - 非 void：先弹出返回值，再截断，最后把返回值压回去
//!
//! **条件跳转**  
//! `JumpIfFalse`/`JumpIfTrue` 只查看栈顶，**不弹出**；  
//! 生成代码的每条路径都要自己 `Pop` 掉条件，否则栈会残留垃圾。
//!
//! **API**  
//! - `run_source`：给测试用的单文件源码（**不能**含 import）
//! - 文件/CLI 请用 `module_loader::run_file`
